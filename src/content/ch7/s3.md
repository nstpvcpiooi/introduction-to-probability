...
Định nghĩa 7.3.1 (Hiệp phương sai). Hiệp phương sai giữa hai biến ngẫu nhiên X và Y là
...
Định lý 7.3.2. Nếu X và Y độc lập, thì chúng là không tương quan.

Chứng minh. Chúng ta sẽ chứng minh trong trường hợp X và Y là liên tục, với hàm mật độ xác suất  $f_{X}$  và  $f_{Y}$ . Vì X và Y độc lập, hàm mật độ xác suất chung của chúng là tích của các hàm mật độ xác suất biên. Theo định lý 2D LOTUS,
...
<div style="text-align: center;"><img src="imgs/img_in_chart_box_194_384_1192_1377.jpg" alt="Image" width="71%" /></div>


### HÌNH 7.9

<div style="text-align: center;">Lấy từ phân phối chung của  $(X, Y)$  dưới các cấu trúc phụ thuộc khác nhau. Góc trên trái: X và Y có tương quan dương. Góc trên phải: X và Y có tương quan âm. Góc dưới trái: X và Y độc lập, do đó không tương quan. Góc dưới phải: Y là hàm xác định của X, nhưng X và Y không tương quan.</div>
...
☑ 7.3.3. Nếu X và Y là độc lập, thì các tính chất của covariance cho
...
Định nghĩa 7.3.4 (Correlation). Correlation giữa các biến ngẫu nhiên X và Y là
...
Định lý 7.3.5 (Các giới hạn của hệ số tương quan). Với bất kỳ các biến ngẫu nhiên X và Y nào,
...
Chứng minh. Không mất tính tổng quát, ta có thể giả sử X và Y có phương sai bằng 1, vì việc thay đổi tỷ lệ không thay đổi hệ số tương quan. Gọi  $\rho = \text{Corr}(X, Y) = \text{Cov}(X, Y)$ . Sử dụng tính chất phương sai không âm, cùng với Tính chất 7 của phương hiệp biến, ta có
...
Ví dụ 7.3.6 (Cực đại và cực tiểu của phân phối mũ). Gọi X và Y là các biến ngẫu nhiên độc lập và đồng phân phối Expo(1). Tìm hệ số tương quan giữa  $\max(X, Y)$  và  $\min(X, Y)$ .
...
Ví dụ 7.3.8 (Phương sai của phân phối siêu hình học). Tìm  $\operatorname{Var}(X)$  cho  $X \sim \operatorname{HGeom}(w, b, n)$ .

## Giải pháp:

Giải thích X như là số bóng trắng trong một mẫu có kích thước n từ một urn có w bóng trắng và b bóng đen. Chúng ta có thể biểu diễn X như là tổng của các biến ngẫu nhiên chỉ báo,  $X = I_1 + \cdots + I_n$ , nơi  $I_j$  là chỉ báo của việc bóng thứ j trong mẫu là trắng. Mỗi  $I_j$  có kỳ vọng  $p = w/(w + b)$  và phương sai  $p(1 - p)$ , nhưng vì các  $I_j$  là phụ thuộc, chúng ta không thể đơn giản cộng phương sai của chúng. Thay vào đó, chúng ta áp dụng tính chất của hiệp phương sai:
...
vì tất cả  $\binom{n}{2}$  cặp chỉ báo có cùng hiệp phương sai nhờ tính đối xứng. Bây giờ chúng ta chỉ cần tìm  $\text{Cov}(I_{1}, I_{2})$ . Theo cầu nối cơ bản,
 ...
 $$\begin{aligned}\operatorname{Cov}(I_{1},I_{2})&=E(I_{1}I_{2})-E(I_{1})E(I_{2})\\&=P(1st và 2nd bóng đều trắng)-P(1st bóng trắng)P(2nd bóng trắng)\\&=\frac{w}{w+b}\cdot\frac{w-1}{w+b-1}-p^{2}.\end{aligned}$$ 
