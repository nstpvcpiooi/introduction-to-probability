...
Xấp xỉ 10.3.2 (Định lý giới hạn trung tâm, dạng xấp xỉ). Với n lớn, phân phối của  $\bar{X}_{n}$  gần似  $\mathcal{N}(\mu,\sigma^{2}/n)$ .

Bằng chứng. Thay đổi mũi tên trong CLT thành  $\sim$ , một ký hiệu phân phối xấp xỉ:
...
Ví dụ 10.3.4 (Hội tụ của Poisson đến Normal). Cho  $Y \sim \text{Pois}(n)$ . Theo Định lý 4.8.1, chúng ta có thể coi Y là tổng của  $n$  r.v.s độc lập và đồng phân phối Pois(1). Do đó, với n lớn,
...
Ví dụ 10.3.5 (Hội tụ của Gamma đến Normal). Cho  $Y \sim \Gamma(\alpha, \lambda)$ . Theo Định lý 8.4.3, chúng ta có thể coi Y là tổng của n r.v.s độc lập và đồng phân phối  $\operatorname{Expo}(\lambda)$ . Do đó, với n lớn,
...
Ví dụ 10.3.6 (Hội tụ của Binomial đến Normal). Cho  $Y \sim \text{Bin}(n, p)$ . Theo Định lý 3.8.8, chúng ta có thể coi Y là tổng của n r.v.s độc lập và đồng phân phối  $\text{Bern}(p)$ . Do đó, với n lớn,
...
Ví dụ 10.3.7 (Chứng khoán biến động). Mỗi ngày, một chứng khoán biến động tăng 70% hoặc giảm 50% giá, với xác suất bằng nhau và các ngày độc lập. Gọi  $Y_n$  là giá cổ phiếu sau n ngày, bắt đầu từ giá trị ban đầu  $Y_0 = 100$ .

(a) Giải thích tại sao  $\log Y_{n}$  gần似 Normal khi n lớn, và nêu các tham số của nó.
...
Định nghĩa 10.4.1 (Phân phối Chi-Square). Gọi  $V = Z_1^2 + \cdots + Z_n^2$  nơi  $Z_1, Z_2, \ldots, Z_n$  là i.i.d.  $\mathcal{N}(0,1)$ . Khi đó V được cho là có phân phối Chi-Square với n bậc tự do. Chúng ta viết điều này là  $V \sim \chi_n^2$ .

Như đã thấy, phân phối  $\chi_{n}^{2}$  là một trường hợp đặc biệt của Gamma.

Định lý 10.4.2. Phân phối  $\chi_{n}^{2}$  là Gamma  $\left(\frac{n}{2},\frac{1}{2}\right)$ .

Chứng minh. Trước tiên, chúng ta xác minh rằng PDF của  $Z_{1}^{2} \sim \chi_{1}^{2}$  bằng với PDF của Gamma  $(\frac{1}{2}, \frac{1}{2})$ : với x > 0,
...
Ví dụ 10.4.3 (Phân phối của phương sai mẫu). Với các  $X_{1}, \ldots, X_{n} \sim \mathcal{N}(\mu, \sigma^{2})$ , phương sai mẫu là biến ngẫu nhiên.
...
Bây giờ lấy hàm sinh momen của cả hai vế. Theo Ví dụ 7.5.9,  $\sum_{j=1}^{n}(Z_j - \bar{Z}_n)^2$  và  $n\bar{Z}_n^2$  độc lập, do đó hàm sinh momen của tổng của chúng là tích của các hàm sinh momen riêng lẻ. Ngoài ra,
 ...
 $$\sum_{j=1}^{n}Z_{j}^{2}\sim\chi_{n}^{2}\mathrm{~a n d~}n\bar{Z}_{n}^{2}\sim\chi_{1}^{2},$$ 
...
VẬY
 ...
 $$\left(\frac{1}{1-2t}\right)^{n/2}=\left(\mathrm{M G F~o f~}\sum_{j=1}^{n}(Z_{j}-\bar{Z}_{n})^{2}\right)\cdot\left(\frac{1}{1-2t}\right)^{1/2}.$$ 
...
Điều này cho thấy
 ...
 $$\left(\mathrm{M G F~o f~}\sum_{j=1}^{n}(Z_{j}-\bar{Z}_{n})^{2}\right)=\left(\frac{1}{1-2t}\right)^{(n-1)/2},$$ 
...
Định nghĩa 10.4.4 (Phân phối Student-t). Cho
...
Định lý 10.4.5 (Tính chất của phân phối Student-t). Phân phối Student-t  $t_{n}$  có các tính chất sau.
...
Bằng chứng. Trong bằng chứng của mỗi tính chất, chúng ta tham khảo Định nghĩa 10.4.4.
