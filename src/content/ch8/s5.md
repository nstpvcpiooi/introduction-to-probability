...
Chúng ta sẽ thực hiện một thay đổi biến trong hai chiều để tìm PDF chung của T và W. Gọi  $t = x + y$ ,  $w = \frac{x}{x+y}$ . Sau đó  $x = tw$ ,  $y = t(1 - w)$ , và
...
Ví dụ 8.5.2 (Kỳ vọng Beta). Với ký hiệu như trên, lưu ý rằng vì T và W là độc lập, chúng là không tương quan:  $E(TW) = E(T)E(W)$ . Viết lại theo X và Y, chúng ta có
...
Ví dụ 8.5.3 (PMF của Beta-Binomial). Cho  $X|p \sim \text{Bin}(n, p)$ , với  $p \sim \text{Beta}(a, b)$ . Như đã đề cập trong Câu chuyện 8.3.3, X có phân phối Beta-Binomial. Tìm phân phối biên của X.

Giải pháp: Gọi  $f(p)$  là PDF của Beta  $(a,b)$ . Sau đó
